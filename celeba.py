...
def main():
    ...
    if args.expid is not None:
        args.root = osp.join(results_path, f"celeba{args.resolution}", args.expid)
    else:
        args.root = osp.join(results_path, f"celeba{args.resolution}")

    model_cls = getattr(load_module(f"models/retreever.py"), "Retreever")
    with open(f"configs/celeba{args.resolution}.yaml", "r") as f:
...
def train(args, model):
    ...
    for epoch in tqdm(range(start_epoch, args.num_epochs + 1)):
        ...
        line = f"ReTreever:{args.expid} epoch {epoch} "
...
def gen_evalset(args):
    ...
    eval_loader = torch.utils.data.DataLoader(
        eval_ds, batch_size=args.eval_batch_size, shuffle=False, num_workers=4
    )
