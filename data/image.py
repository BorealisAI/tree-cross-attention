...
def img_to_task(
    ...
    num_tar = (
        max_num_points - num_ctx
        if target_all
        else torch.randint(
            low=3, high=min(max_num_points - num_ctx, max_num_target_points), size=[1]
        ).item()
    )
    num_points = num_ctx + num_tar
    idxs = (
        torch.cuda.FloatTensor(B, num_pixels)
        .uniform_()
        .argsort(-1)[..., :num_points]
        .to(img.device)
    )
    x1, x2 = idxs // W, idxs % W
    batch.x = torch.stack(
        [2 * x1.float() / (H - 1) - 1, 2 * x2.float() / (W - 1) - 1], -1
    ).to(img.device)
    batch.y = (
        torch.gather(img, -1, idxs.unsqueeze(-2).repeat(1, C, 1)).transpose(-2, -1)
        - 0.5
    ).to(img.device)
